...
//			Log.d("token", token);
			myHttpThreadPost = new HttpThreadPost(SubmitActivity.URL
					+ "getHistory.php", params, HttpThreadPost.HISTORY, handler);
	...
	private void creatNotification() {
		...
		mBuilder.setContentTitle("Your task is done!")//设置通知栏标题
		.setContentText("Your task is done! \nPlease check and get the result.").setContentIntent(pendingIntent) //设置通知栏点击意图
	...
	private void saveAndCheckStauts(String listJson) {
		try {
			...
			for (int i = 0; i < jsonObjs.length(); i++) {
				...
				String statusNum = ((JSONObject) jsonObjs.opt(i)).getString("status");

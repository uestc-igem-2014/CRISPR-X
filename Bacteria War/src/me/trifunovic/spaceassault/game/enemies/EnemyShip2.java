...
public class EnemyShip2 extends AnimatedSprite implements IEnemy {
	...
	public EnemyShip2(float pX, float pY, TiledTextureRegion pTiledTextureRegion, final Engine engine) {
		super(pX, pY, pTiledTextureRegion);
		mEngine = engine;
		killed=false;
		health = 2;
		GameActivity.enemies2.add(this);
	...
	public static EnemyShip2 reuse() {
		final EnemyShip2 enemy = GameActivity.enemiesToReuse2.get(0);
		enemy.killed = false;
		health = 2;
		GameActivity.enemies2.add(enemy);
		GameActivity.enemiesToReuse2.remove(enemy);
		return enemy;
	}
				...
				new IShapeModifierListener() {
					...
					public void onModifierFinished(
						...
						mEngine.runOnUpdateThread(new Runnable() {
							public void run() {
								mEngine.getScene().getBottomLayer().removeEntity(((EnemyShip2) enemy));
								GameActivity.enemiesToReuse2.add(0,(EnemyShip2) enemy);
								GameActivity.enemies2.remove((EnemyShip2) enemy);
								((EnemyShip2) enemy).setScale(1);
							}
						});
					}
				...
				null, new IPathModifierListener() {
					...
					public void onWaypointPassed(
						...
						if (pWaypointIndex == 6) {
							mEngine.runOnUpdateThread(new Runnable() {
								public void run() {
									((EnemyShip2) pShape).removeFromScene();
								}
							});
						}

						if (pWaypointIndex != 0 && pWaypointIndex != 6)
							((EnemyShip2) pShape).fire();
					}
